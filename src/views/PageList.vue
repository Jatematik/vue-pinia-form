<template>
  ...
  <app-message v-if="!interviews.length" severity="info">Нет добавленных собеседований</app-message>
  <div v-else>
    ...
    <app-datatable :value="interviews" :loading="isLoading">
        ...
        <template #body="slotProps">
          <div class="contacts">
            ...
            <a
              ...
              :href="`https://whatsapp.com/${slotProps.data.contactWhatsApp}`"
              target="_blank"
...
const submitFilter = async () => {
  ...
  isLoading.value = true
  const listInterviews = await getAllInterviews(true)
...
const clearFilter = async () => {
  isLoading.value = true
  const listInterviews = await getAllInterviews(false)
  interviews.value = listInterviews
  isLoading.value = false
